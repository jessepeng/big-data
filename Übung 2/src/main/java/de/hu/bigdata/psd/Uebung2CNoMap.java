...
public class Uebung2CNoMap {
	...
	public static void main(String[] args) throws Exception {
		...
		String orderStatusFilter = args[2];
     ...
     */
	private static DataSet<Tuple2<Long, Double>> sumOrders(String ordersPfad, String lineitemsPfad, String orderStatusFilter, Date finalShipDateToFilter, Date finalShipDateFromFilter, ExecutionEnvironment env) {
		DataSet<Tuple2<Long, String>> orders = env.readCsvFile(ordersPfad).fieldDelimiter("|").includeFields("101").types(Long.class, String.class).setParallelism(1);
		DataSet<Tuple3<Long, Double, Date>> lineItems = env.readTextFile(lineitemsPfad).map(new LineItemsSplitter());
		DataSet<Tuple2<Long, String>> filteredOrders = orders.filter(tuple -> tuple.f1.equals(orderStatusFilter));
