...
public class Uebung2CAnnotated {
	...
	private static DataSet<Tuple2<Long, Double>> sumOrders(String ordersPfad, String lineitemsPfad, Character orderStatusFilter, Date finalShipDateToFilter, Date finalShipDateFromFilter, ExecutionEnvironment env) {
		...
		DataSet<Tuple2<Long, Character>> filteredOrders = orders.filter(new OrderFilterFunction(orderStatusFilter));
		DataSet<Tuple3<Long, Double, Date>> filteredLineItems = lineItems.filter(new LineItemsFilterFunction(finalShipDateToFilter, finalShipDateFromFilter));
		DataSet<Tuple2<Long, Double>> joinedOrders = filteredOrders.joinWithHuge(filteredLineItems).where(0).equalTo(0).with(new OrderLineItemsJoinFunction());
	...
	private static final class OrderLineItemsJoinFunction implements JoinFunction<Tuple2<Long, Character>, Tuple3<Long, Double, Date>, Tuple2<Long, Double>> {

		@Override
		public Tuple2<Long, Double> join(Tuple2<Long, Character> tuple1, Tuple3<Long, Double, Date> tuple2) throws Exception {
			return new Tuple2<Long, Double>(tuple1.f0, tuple2.f1);
		}
	}

	@FunctionAnnotation.ReadFields("f2")
	private static final class LineItemsFilterFunction implements FilterFunction<Tuple3<Long, Double, Date>> {

		private Date finalShipDateToFilter;
		private Date finalShipDateFromFilter;

		LineItemsFilterFunction(Date sipDateToFilter, Date shipDateFromFilter) {
			this.finalShipDateFromFilter = shipDateFromFilter;
			this.finalShipDateToFilter = sipDateToFilter;
		}

		@Override
		public boolean filter(Tuple3<Long, Double, Date> tuple) throws Exception {
			return tuple.f2.before(finalShipDateToFilter) && !tuple.f2.before(finalShipDateFromFilter);
		}
	}

	@FunctionAnnotation.ReadFields("f1")
	private static final class OrderFilterFunction implements FilterFunction<Tuple2<Long, Character>> {

		private Character orderStatusFilter;

		OrderFilterFunction(Character orderStatusFilter) {
			this.orderStatusFilter = orderStatusFilter;
		}

		@Override
		public boolean filter(Tuple2<Long, Character> tuple) throws Exception {
			return tuple.f1.equals(orderStatusFilter);
		}
	}
