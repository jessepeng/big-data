...
public class Uebung2CSortMerge {
	...
	private static DataSet<Tuple2<Long, Double>> sumOrders(String ordersPfad, String lineitemsPfad, Character orderStatusFilter, Date finalShipDateToFilter, Date finalShipDateFromFilter, ExecutionEnvironment env) {
		...
		DataSet<Tuple2<Long, Double>> joinedOrders = filteredOrders.join(filteredLineItems, JoinOperatorBase.JoinHint.REPARTITION_SORT_MERGE).where(0).equalTo(0).with((tuple1, tuple2) -> new Tuple2<Long, Double>(tuple1.f0, tuple2.f1));
